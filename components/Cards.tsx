...
export const FeaturedCard = ({onPress}: Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-80 relative'>
      <Image source={images.baju} className='size-full rounded-2xl ' resizeMode='contain'/>
      ...
      <View className='flex flex-col items-start absolute bottom-5 inset-x-5'>
        ...
        <Text className='text-base font-rubik text-white'> lorem ipsum lambada</Text>
...
export const Card= ({onPress}: Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'>
      ...
      <Image source={images.baju} className='w-full h-40 rounded-lg' />
